...
class TaxBand {
  constructor(from, to, rate) {
    this.from = from;
    this.to = to;       // null means no upper limit
    this.rate = rate;    // e.g. 0.05 for 5%
  }

  /**
   * Calculate tax owed in this band for a given property price.
   * @param {number} price - Total property price in £
   * @returns {number} Tax owed for this band
   */
  calculate(price) {
...
class SDLTCalculator {
   ...
   * @param {number} price - Property price as an integer in £
   * @returns {SDLTResult}
   */
  calculate(price) {
    ...
    const breakdown = this.bands.map(band => ({
      band: band.toString(),
